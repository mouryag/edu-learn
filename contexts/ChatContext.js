...
export const ChatProvider = ({ children }) => {
  ...
  const createNewChat = useCallback(async () => {
    ...
    const existingEmptyChat = state.chats.find(c => (c.messages?.length ?? 0) === 0)
    if (existingEmptyChat) {
      dispatch({ type: 'SET_CURRENT_CHAT', payload: existingEmptyChat.id })
      return { user_id: state.user_id, session_id: existingEmptyChat.id }
    }
